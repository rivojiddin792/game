...
// Scale dinamik hisoblanadi
var scale = Math.min(window.innerWidth / LEVELS[0][0].length, window.innerHeight / LEVELS[0].length) * 0.9;
...
DOMDisplay.prototype.scrollPlayerIntoView = function () {
    ...
    if (center.x < left.xlength + margin) this.wrap.scrollLeft = center.x - margin;
    else if (center.x > right.x - margin) this.wrap.scrollLeft = center.x + margin;
    if (center.y < top.y + margin) this.wrap.scrollTop = center.y - margin;
    else if (center.y > bottom.y - margin) this.wrap.scrollTop = center.y + margin - height;
}
...
// Sensorli tugmalar uchun boshqaruv
function trackTouchKeys() {
    var pressed = Object.create(null);
...
// Klaviatura va sensorli boshqaruvlarni birlashtirish
function combineInputs(keyboard, touch) {
    var combined = Object.create(null);
    return new Proxy(combined, {
        get: function (target, key) {
            return keyboard[key] || touch[key] || false;
        },
...
var keyboardInput = trackKeys(arrowCodes);
var touchInput = trackTouchKeys();
var arrows = combineInputs(keyboardInput, touchInput);
